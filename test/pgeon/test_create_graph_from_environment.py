...
from pgeon import GraphRepresentation, PolicyGraph, Predicate
...
class TestCreateGraphFromEnvironment(unittest.TestCase):
    ...
    def test_fit_from_agent_and_env(self):
        ...
        self.assertAlmostEqual(
            pg.graph.edges[(p[0], p[1], 0)]["probability"], 1, delta=0.001
        )
        self.assertAlmostEqual(
            pg.graph.edges[(p[1], p[2], 0)]["probability"], 1, delta=0.001
        )
        self.assertAlmostEqual(
            pg.graph.edges[(p[2], p[3], 0)]["probability"], 1, delta=0.001
        )
        self.assertAlmostEqual(
            pg.graph.edges[(p[3], p[0], 0)]["probability"], 1, delta=0.001
        )
